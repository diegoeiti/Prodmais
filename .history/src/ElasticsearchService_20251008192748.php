...
class ElasticsearchService
{
    ...
    private function searchFallback(array $filters = [], int $size = 50): array
    {
        ...
        $sampleData = [
            [
                'id' => 'demo_1',
                'title' => 'Análise de Sistemas de Gestão Universitária na Era Digital',
                'researcher_name' => 'Dr. João Silva Santos',
                'type' => 'artigo',
                'subtype' => 'completo',
                'year' => 2024,
                ...
                'qualis' => 'B1',
                'authors' => [
                    ['name' => 'João Silva Santos', 'citation_name' => 'SANTOS, J. S.']
                ]
            ],
            [
                'id' => 'demo_2',
                'title' => 'Direitos Fundamentais na Constituição Digital',
                'researcher_name' => 'Dra. Maria Oliveira Costa',
                'type' => 'livro',
                'year' => 2023,
                'publisher' => 'Editora Jurídica UMC',
                'institution' => 'Universidade de Mogi das Cruzes',
                'language' => 'pt',
                'programa_umc' => 'mestrado_direito',
                'authors' => [
                    ['name' => 'Maria Oliveira Costa', 'citation_name' => 'COSTA, M. O.']
                ]
            ],
            [
                'id' => 'demo_3',
                'title' => 'Machine Learning Applications in Educational Systems',
                'researcher_name' => 'Dr. Carlos Roberto Lima',
                'type' => 'artigo',
                'subtype' => 'completo',
                'year' => 2024,
                ...
                'qualis' => 'A2',
                'authors' => [
                    ['name' => 'Carlos Roberto Lima', 'citation_name' => 'LIMA, C. R.']
                ]
            ],
            [
                'id' => 'demo_4',
                'title' => 'Psicologia Cognitiva e Aprendizagem Digital',
                'researcher_name' => 'Dra. Ana Paula Ferreira',
                'type' => 'capitulo',
                'year' => 2023,
                'book_title' => 'Inovações em Psicologia Educacional',
                'publisher' => 'Editora Científica',
                'institution' => 'Universidade de Mogi das Cruzes',
                'language' => 'pt',
                'programa_umc' => 'mestrado_psicologia',
                'authors' => [
                    ['name' => 'Ana Paula Ferreira', 'citation_name' => 'FERREIRA, A. P.']
                ]
            ],
            [
                'id' => 'demo_5',
                'title' => 'Sustentabilidade no Ensino Superior: Práticas UMC',
                'researcher_name' => 'Dr. Pedro Henrique Santos',
                'type' => 'evento',
                'year' => 2024,
                'event_name' => 'Congresso Internacional de Educação',
                'institution' => 'Universidade de Mogi das Cruzes',
                'language' => 'pt',
                'programa_umc' => 'mestrado_educacao',
                'authors' => [
                    ['name' => 'Pedro Henrique Santos', 'citation_name' => 'SANTOS, P. H.']
                ]
            ]
        ];

        // Filtra os dados baseado nos parâmetros
        $filteredData = array_filter($sampleData, function($item) use ($filters) {
            // Filtro por texto
            if (!empty($filters['q'])) {
                $searchText = strtolower($filters['q']);
                $itemText = strtolower($item['title'] . ' ' . $item['researcher_name']);
                if (strpos($itemText, $searchText) === false) {
                    return false;
                }
            }

            // Filtro por programa UMC
            if (!empty($filters['program']) && $filters['program'] !== $item['programa_umc']) {
                return false;
            }

            // Filtro por tipo
            if (!empty($filters['type']) && $filters['type'] !== $item['type']) {
                return false;
            }

            // Filtro por ano
            if (!empty($filters['year']) && $filters['year'] != $item['year']) {
                return false;
            }
        ...
        $filteredData = array_slice($filteredData, 0, $size);

        return [
            'hits' => [
                'total' => ['value' => count($filteredData)],
